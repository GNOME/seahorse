...
public class Seahorse.Sidebar : Gtk.ScrolledWindow {
    ...
    private void ensure_sidebar_pixbufs() {
        ...
        Gtk.IconInfo? icon_info = icon_theme.lookup_by_gicon(icon, this.action_button_size, Gtk.IconLookupFlags.GENERIC_FALLBACK);
        if (icon_info == null)
            return;
        try {
        ...
        icon_info = icon_theme.lookup_by_gicon(icon, this.action_button_size, Gtk.IconLookupFlags.GENERIC_FALLBACK);
        if (icon_info == null)
            return;
        try {
