...
seahorse_decrypt_verify_file_new (SeahorseContext *sctx)
{
	...
	swidget = seahorse_widget_new ("decrypt-verify-file", sctx);
...
seahorse_decrypt_verify_file (SeahorseContext *sctx, const gchar *file)
{
	...
	/* show signatures and modal info dialog */
	if (seahorse_ops_file_decrypt_verify (sctx, file, &status)) {
		...
		seahorse_util_show_error (NULL, g_strdup_printf (
			_("Could not decrypt and verify %s"), file));
		return FALSE;
	}
}
