...
get_http_server_uri (SeahorseHKPSource *self, const char *path)
{
    ...
    g_autofree gchar *conf_uri = NULL;
    ...
    if (strncasecmp (conf_uri, "hkp:", 4) == 0) {
        g_autofree gchar *t = g_strdup_printf ("http:%s", conf_uri + 4);
        uri = soup_uri_new (t);
    } else if (strncasecmp (conf_uri, "hkps:", 5) == 0) {
          g_autofree gchar *t = g_strdup_printf ("https:%s", conf_uri + 5);
          uri = soup_uri_new (t);
    }
